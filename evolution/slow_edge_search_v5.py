...
SYMBOL = "BTC"
REPORT_PATH = Path("slow_edge_search_v5_result.json")
...
def main() -> None:
    print("ORACLE X — SLOW 1H/4H EDGE SEARCH V5", flush=True)
    ...
    boundaries = [timestamps[int(len(points) * fraction)] for fraction in (0.50, 0.625, 0.75, 0.875)]
    final_ts = timestamps[-1] + 3600
    folds = list(zip(boundaries, boundaries[1:] + [final_ts]))
    ...
    for index, candidate in enumerate(candidates, 1):
        ...
        ranked.append((score, candidate, evaluations))
        if index % 36 == 0:
            print(f"Evaluated {index}/{len(candidates)}", flush=True)

    ranked.sort(key=lambda item: item[0], reverse=True)
    winner_score, winner, winner_evaluations = ranked[0]
    final_diagnostic = evaluation_dict(winner_evaluations[3])
    selection_combined = combined(winner_evaluations[:3])
    all_combined = combined(winner_evaluations)
    stress_evaluations = [
        evaluate(points, rows_15m, winner, start, end, STRESS_FEE_BPS, STRESS_SLIPPAGE_BPS)
        for start, end in folds
    ]
    stress = combined(stress_evaluations)
    selection_metrics = [trade_metrics(value.trades) for value in winner_evaluations[:3]]
    gate = (
        ...
        and final_diagnostic["trades"] >= 8
        and final_diagnostic["total_r"] > 0
        and final_diagnostic["profit_factor"] >= 1.10
        and stress["total_r"] > 0
        and stress["profit_factor"] >= 1.10
        and stress["max_drawdown_r"] <= 12.0
    )
    status = "HISTORICAL_CANDIDATE_ONLY" if gate else "NO_EDGE"

    report = {
        "status": status,
        "warning": "Historical periods are exposed; only new forward paper data can authorize live trading.",
        ...
        "coverage": {"start": iso(timestamps[0]), "end": iso(final_ts)},
        "winner": asdict(winner),
        "winner_score": round(winner_score, 6),
        "selection_folds": [
            {**evaluation_dict(value), "start": iso(start), "end": iso(end)}
            for value, (start, end) in zip(winner_evaluations[:3], folds[:3])
        ],
        "selection_combined_at_10bps": selection_combined,
        "final_diagnostic_at_10bps": final_diagnostic,
        "all_walk_forward_at_10bps": all_combined,
        "all_walk_forward_stress_at_12bps": stress,
        ...
        "top_candidates": [
            {
                ...
                "final_diagnostic": evaluation_dict(evaluations[3]),
            }
